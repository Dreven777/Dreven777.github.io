...
function defUpperStr(msg){
    if(msg === undefined) return 'DEFAULT TEXT';
    else return String(msg).toUpperCase();
}
 ...
 */
function evenFn(count){
    let arr = [];
    for(let i = 0; i <= count; i++){
        if((i%2) === 0 && i != 0) arr.push(i);
    }
    return arr;
}
...
function ageClassification(n){
    ...
    return n <= 24 ? 'Дитинство' : 
        ...
        n <= 65 && n > 44 ? 'Зрілість' :
        n <= 75 && n > 65 ? 'Старість' :
        n <= 90 && n > 75 ? 'Довголіття' :
        n <= 122 && n > 90 ? 'Рекорд' : null;
}
